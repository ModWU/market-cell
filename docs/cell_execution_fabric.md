# MarketCell Cell Execution Fabric v0.6
...
窗口快照包含样本运行数、P50 / P95 / P99、失败率、重试率和最近状态。Planner 把实际使用的完整快照写入 ExecutionPlan metadata，避免 placement 使用无法复盘的隐式“全历史”。窗口外 trace 不参与选择，新 formula_version 或 implementation_id 默认从 no_history 开始。
...
AnalysisRun 保存 ExecutionPlan、plan execution、runtime trace、单次 summary、跨运行窗口快照和写入审计，用于复盘、性能分析和后续调度优化。
...
Graph、Organ、Graph Validator、计划、input resolver、binding、catalog、placement、Plan Validator、plan-driven coordinator、executor、trace、summary、Runtime Summary Store 和固定性能基线已有本地参考实现。
